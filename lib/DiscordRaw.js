...
const InflateStream = require('./InflateStream');
...
class DiscordRaw extends EventEmitter {
	static get API_URL() {return 'https://discord.com/api';}
	...
	constructor(opts) {
		...
		this._userMode = typeof(opts.userMode) !== 'undefined' ? !!opts.userMode : false;
		...
		this._extendProperties = typeof(opts.extendProperties) === 'object' ? opts.extendProperties : {};
	...
	_getGatewayURL(callback) {
		...
		this._request(DiscordRaw.ENDPOINT_GATEWAY, (err, data) => {
			...
			this._gatewayURL = data.url + '/?encoding=json&v=' + DiscordRaw.GATEWAY_VERSION + '&compress=zlib-stream';
	...
	_connect() {
		...
		this._getGatewayURL((err, url) => {
			...
			// Always start a new inflate context
			this._inflate = new InflateStream((err, json) => {
				if (err) {
					if (this._logLevel >= DiscordRaw.LOGLEVEL_ERROR) {
			...
			this._ws.on('close', closeCode => {
				...
				setTimeout(() => this._connect(), Math.floor(Math.random() * 1000) + 1000);
			});

			this._ws.on('message', (msg, flags) => {
				if (flags.binary) {
					// The inflate callback is responsible for handling errors and messages.
					this._inflate.push(msg);
				} else {
	...
	_handleMessage(event) {
		...
		switch (event.op) {
			...
			case DiscordRaw.OPCODE_INVALID_SESSION:
				...
				setTimeout(() => this._sendIdentify(), Math.floor(Math.random() * 500) + 200);
	...
	_sendHeartbeat() {
		if (this._lastHeartbeat !== null && this._lastHeartbeat > this._lastHeartbeatAck) {
			if (this._logLevel >= DiscordRaw.LOGLEVEL_INFO) console.info(`WebSocket timed out due to missing heartbeat acknowledgement`);
