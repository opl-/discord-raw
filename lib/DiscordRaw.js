...
class DiscordRaw extends EventEmitter {
	...
	static get GATEWAY_VERSION() {return 6;}
	...
	static get OPCODE_STATUS_UPDATE() {return 3;}
	...
	constructor(opts) {
		...
		this._noResume = !!opts.noResume || false;
	...
	_getGatewayURL(callback) {
		...
		this._request(DiscordRaw.ENDPOINT_GATEWAY, (err, data) => {
			...
			this._gatewayURL = data.url + '/?v=' + DiscordRaw.GATEWAY_VERSION + '&encoding=json';
	...
	_handleMessage(event) {
		...
		switch (event.op) {
			case DiscordRaw.OPCODE_DISPATCH:
				...
				break;
			case DiscordRaw.OPCODE_STATUS_UPDATE:
				break;
